...
class BiLSTM(nn.Module):
    ...
    def bayes(self,ht,mc_pro,mc_round):
        # 贝叶斯文章的核心部分，仅在测试阶段，mc_pro dropout的概率，mc_round forword次数
        dropout = nn.Dropout(p=mc_pro)
        logits_list,probs_list = [],[]
        for i in range(mc_round):
            ht_tmp = dropout(ht)
            logits = self.fc(ht_tmp)
        ...
        # etp = probs.mul(torch.log(probs))
        # etp = torch.mean(torch.mean(etp, dim=2),dim=0)
        logits = torch.tensor(logits_list)
        logits = torch.mean(logits, dim=0)
        return probs,logits

    def forward(self, seq, label=None, mode='ind_pre',mc_dropout=False, mc_round=0,mc_pro = 0.5):
        if mode in ['ind_pre','finetune',"energy_onemargin_loss","energy_twomargin_loss"]:
            if self.use_bert:
            ...
            ht = torch.cat((ht[0].squeeze(0), ht[1].squeeze(0)), dim=1)
            if mc_dropout:
                probs,logits = self.bayes(ht,mc_pro,mc_round)
