...
def parse_args():
    ...
    parser.add_argument("--loss_method", type=str, choices=["energy_twomargin_loss","energy_onemargin_loss"], default = None,help="Specify loss method, 'energy_two means the energyloss has two edge hyperparameters'")

    # bayes
    parser.add_argument("--mc_dropout", action="store_true", help="whether to use MC dropout or not.")
    parser.add_argument("--mc_round", type=int, default=100, help="Rounds of MC dropout")
    parser.add_argument("--mc_pro", type=float, default=0.5, help="Rounds of MC dropout probability")
...
if  args.dataset in ["CLINC_OOD","CLINC_OOD_full_20","CLINC_OOD_full_40","CLINC_OOD_full_60","CLINC_OOD_full_80","CLINC_OOD_full","CLINC_OOD_small","CLINC_OOD_coarse","CLINC_OOD_coarse_100","CLINC_OOD_coarse_real"]:
...
n_class_unseen = len(y_cols_unseen)
# "...-raw" contains "unseen" and IND data; where y_train_raw's label is unseen
...
'''
y_train_unseen_idx = le.transform(y_train_unseen)
y_valid_unseen_idx = le.transform(y_valid_unseen)
y_test_unseen_idx = le.transform(y_test_unseen)
y_train_unseen_onehot = to_categorical(y_train_unseen_idx)
y_valid_unseen_onehot = to_categorical(y_valid_unseen_idx)
y_test_unseen_onehot = to_categorical(y_test_unseen_idx)

y_train_raw_idx = le.transform(y_train_raw)
y_valid_raw_idx = le.transform(y_valid_raw)
y_test_raw_idx = le.transform(y_test_raw)
y_train_raw_onehot = to_categorical(y_train_raw_idx)
y_valid_raw_onehot = to_categorical(y_valid_raw_idx)
y_test_raw_onehot = to_categorical(y_test_raw_idx)
'''
train_data_raw = (X_train_raw, y_train_seen_onehot)
train_data_seen = (X_train_seen,y_train_seen_onehot)
valid_data_raw = (X_valid_raw, y_valid_seen_onehot)
valid_data_seen = (X_valid_seen, y_valid_seen_onehot)
valid_data_unseen = (X_valid_unseen,y_valid_unseen)
test_data_raw = (X_test_raw, y_test_seen_onehot)
...
if args.mode in ["train", "both"]:
    ...
    if args.experiment_No:
        output_dir = os.path.join(args.output_dir, f"{dataset}-{unseen_classes}-{args.train_class_num}-{args.seed}-{args.loss_method}-{args.experiment_No}")
    else:
        output_dir = os.path.join(args.output_dir, f"{dataset}-{unseen_classes}-{args.train_class_num}-{args.seed}-{args.loss_method}-{timestamp}")
        ...
        for j, (seq, label) in enumerate(train_iterator): # seq (bactsize,n_class)
            if args.cuda:
                if not USE_BERT:
                    seq = seq.cuda()
                label = label.cuda()
            if not args.loss_method:
                loss = model(seq, label, mode='finetune')
            else:
                loss = model(seq, label, mode=args.loss_method)
...
if args.mode in ["test", "both"]:
    ...
    if args.model_dir:
        ...
        model_dir = os.path.join(args.output_dir, f"{dataset}-{unseen_classes}-{args.train_class_num}-{args.seed}-{args.loss_method}-{args.experiment_No}")
    ...
    train_loader_seen = DataLoader(train_data_seen, BATCH_SIZE,use_bert=USE_BERT, raw_text=text_train_seen)
    ...
    for j, (seq, label) in enumerate(valid_loader_unseen):
        ...
        output = model(seq,  None, mode = 'test', mc_dropout=args.mc_dropout, mc_round=args.mc_round,mc_pro = args.mc_pro)
        if j > 0:
    ...
    for detect_method in detect_method_list:
        ...
        if detect_method == "msp":
            threshold = 0
            print("********************************* detect_method = msp *********************************")
            method = 'MSP '

            valid_seen_score = prob_valid_seen.max(axis=1) * -1.0
            valid_unseen_score = prob_valid_unseen.max(axis=1) * -1.0
            test_seen_score = prob_test_seen.max(axis=1) * -1.0
            test_unseen_score = prob_test_unseen.max(axis=1) * -1.0

            if args.threshold:
                threshold = args.threshold
            else:
                threshold = -1.0 * estimate_best_threshold(valid_seen_score, valid_unseen_score)
            df_seen = pd.DataFrame(prob_test, columns=le.classes_)
            df_seen['unseen'] = 0
            
            # plot_distribution(prob_test_unseen.max(axis=1),prob_test_unseen.max(axis=1),"msp.png")
            ...
            f, f_seen, acc_in , r_seen, f_unseen, p_unseen, r_unseen = get_score(cm)
